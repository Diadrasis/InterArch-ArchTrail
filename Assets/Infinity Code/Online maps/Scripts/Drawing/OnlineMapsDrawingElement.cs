    ...
    protected void DrawLineToBuffer(Color32[] buffer, OnlineMapsVector2i bufferPosition, int bufferWidth, int bufferHeight, int zoom, IEnumerable points, Color32 color, float width, bool closed, bool invertY)
    {
        ...
        lock (points)
        {
            foreach (object p in points)
            {
                ...
                if (drawPart)
                {
                    if (p1tx < bx1 && p2tx < bx1)
                    {
                        p1tx += max;
                        p2tx += max;
                    }
                    else if (p1tx > bx2 && p2tx > bx2)
                    {
                        p1tx -= max;
                        p2tx -= max;
                    }
                    ...
                    {
                        if (p1tx - p2tx > halfMax)
                        {
                            double ty = Get180IntersectionY(p1tx, p1ty, p2tx + max, p2ty, max);
                            DrawLinePartToBuffer(buffer, bufferPosition, bufferWidth, bufferHeight, color, sx, sy, p1tx, p1ty, max - double.Epsilon, ty, max, w, invertY);
                            DrawLinePartToBuffer(buffer, bufferPosition, bufferWidth, bufferHeight, color, sx, sy, double.Epsilon, ty, p2tx, p2ty, max, w, invertY);
                        }
                        else if (p1tx - p2tx < -halfMax)
                        {
                            double ty = Get180IntersectionY(p1tx + max, p1ty, p2tx, p2ty, max);
                            DrawLinePartToBuffer(buffer, bufferPosition, bufferWidth, bufferHeight, color, sx, sy, p2tx, p2ty, max - double.Epsilon, ty, max, w, invertY);
                            DrawLinePartToBuffer(buffer, bufferPosition, bufferWidth, bufferHeight, color, sx, sy, 0, ty, p1tx, p1ty, max, w, invertY);
                        }
                        else DrawLinePartToBuffer(buffer, bufferPosition, bufferWidth, bufferHeight, color, sx, sy, p1tx, p1ty, p2tx, p2ty, max, w, invertY);
        ...
        if (closed && i > 0)
        {
            ...
            {
                if (p1tx - p2tx > max / 2) p2tx += max;
                else if (p1tx - p2tx < max / -2) p1tx += max;

                DrawLinePartToBuffer(buffer, bufferPosition, bufferWidth, bufferHeight, color, sx, sy, p1tx, p1ty, p2tx, p2ty, max, w, invertY);
            }
        }
    }

    private double Get180IntersectionY(double p1tx, double p1ty, double p2tx, double p2ty, int max)
    {
        double a = Math.Abs(p2ty - p1ty);
        double b = Math.Abs(p2tx - p1tx);
        double A = Math.Atan2(a, b);
        return Math.Max(p1ty, p2ty) - (max - Math.Min(p1tx, p2tx)) * Math.Tan(A);
    }

    private static void DrawLinePartToBuffer(Color32[] buffer, OnlineMapsVector2i bufferPosition, int bufferWidth, int bufferHeight, Color32 color, double sx, double sy, double p1tx, double p1ty, double p2tx, double p2ty, int maxX, int w, bool invertY)
    {
        int bpx = bufferPosition.x;
        int bex = bpx + (bufferWidth >> 8);
        if (p1tx > bex && p2tx > bex)
        {
            p1tx -= maxX;
            p2tx -= maxX;
        }
        else if (p1tx < bpx && p2tx < bpx)
        {
            p1tx += maxX;
            p2tx += maxX;
        }
        if ((p1tx < bpx && p2tx < bpx) || (p1tx > bpx + (bufferWidth >> 8) && p2tx > bpx + (bufferWidth >> 8))) return;
        ...
        if (Math.Sqrt((p1tx - p2tx) * (p1tx - p2tx) + (p1ty - p2ty) * (p1ty - p2ty)) > 0.2)
        {
            ...
            DrawLinePartToBuffer(buffer, bufferPosition, bufferWidth, bufferHeight, color, sx, sy, p1tx, p1ty, p3tx, p3ty, maxX, w, invertY);
            DrawLinePartToBuffer(buffer, bufferPosition, bufferWidth, bufferHeight, color, sx, sy, p3tx, p3ty, p2tx, p2ty, maxX, w, invertY);
            return;
        }
        ...
        bool isEntireWorld = maxX * OnlineMapsUtils.tileSize == api.width;
        int maxK = isEntireWorld ? 2 : 1;
        ...
        for (int k = 0; k < maxK; k++)
        {
            ...
            int c = maxX * OnlineMapsUtils.tileSize;
            stX += c;
            istx += c;
            fromX += c;
            toX += c;
