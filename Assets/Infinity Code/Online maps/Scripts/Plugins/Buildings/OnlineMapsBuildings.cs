...
public class OnlineMapsBuildings : MonoBehaviour
{
    ...
    {
        Vector2 s = OnlineMaps.instance.tilesetSize;
        Vector3 c = new Vector3(s.x / building.initialSizeInScene.x, 1, s.y / building.initialSizeInScene.y);
        c.y = (c.x + c.z) / 2;

        if (building.initialZoom == map.zoom) building.transform.localScale = c;
        else if (building.initialZoom < map.zoom) building.transform.localScale = c * (1 << map.zoom - building.initialZoom);
        else if (building.initialZoom > map.zoom) building.transform.localScale = c / (1 << building.initialZoom - map.zoom);
    }
}
